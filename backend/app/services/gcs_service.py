import datetime
import os
import pathlib
from google.cloud import storage
from app.core.config import settings


class GCSService:
    def __init__(self):
        self.use_local = False
        creds_path = os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
        
        if not creds_path or not os.path.exists(creds_path):
            self.use_local = True
            self.local_root = pathlib.Path("/app/data/local_storage")
            self.local_root.mkdir(parents=True, exist_ok=True)
            return

        try:
            self.client = storage.Client()
            self.bucket = self.client.bucket(settings.GCS_BUCKET_NAME)
        except Exception:
            self.use_local = True
            self.local_root = pathlib.Path("/app/data/local_storage")
            self.local_root.mkdir(parents=True, exist_ok=True)

    def upload_file(self, file_content: bytes, destination_blob_name: str) -> str:
        if self.use_local:
            local_path = self.local_root / destination_blob_name
            local_path.parent.mkdir(parents=True, exist_ok=True)
            with open(local_path, "wb") as f:
                f.write(file_content)
            return f"local://{destination_blob_name}"
            
        blob = self.bucket.blob(destination_blob_name)
        blob.upload_from_string(file_content)
        return f"gs://{settings.GCS_BUCKET_NAME}/{destination_blob_name}"

    def generate_signed_url(self, blob_name: str, expiration_minutes: int = 60) -> str:
        if self.use_local:
            return f"http://localhost:8000/api/v1/uploads/local/{blob_name}"

        blob = self.bucket.blob(blob_name)
        url = blob.generate_signed_url(
            version="v4",
            expiration=datetime.timedelta(minutes=expiration_minutes),
            method="GET",
        )
        return url
